...
namespace ams::i2c::driver::impl {
    ...
    II2cDriver* FindDriver(DeviceCode bus_code) {
        for (auto& itr : GetI2cDriverList()) {
            if (itr.SafeCastTo<II2cDriver>().GetDeviceCode() == bus_code)
            {
                return &itr.SafeCastTo<II2cDriver>();
            }
        }
        return nullptr;
    }
