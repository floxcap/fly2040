...
void write_data(int block, const uint8_t * data, int size) {
    for (int i = 0; i < size; i += 512) {
        ...
        {
            if (write_done)
                finish_with_code(9, 4);
            else
                finish_with_code(10, 4);
        }
    }
}
    ...
    for (int i = 0; i < 0x2800; i += 0x200) {
        ...
        if (!was_success) {
            if (was_read && !was_write)
                finish_with_code(13, 4);
            else if (was_write)
                finish_with_code(14, 4);
            else
                finish_with_code(15, 4);
        }
    }
}
